...
const url = 'https://bharathyundai.com/venue-accessories/';
const outputDir = path.join(__dirname, '..', 'public', 'images', 'accessories', 'venue');
...
const accessories = [
  { name: 'HEAD LAMP', keywords: ['head', 'lamp'] },
  { name: 'DOOR SIDE MOULDING', keywords: ['door', 'side', 'moulding', 'molding'] },
  { name: 'WINDOW LINE CHROME', keywords: ['window', 'line', 'chrome'] },
  { name: 'UPPER TRUNK', keywords: ['upper', 'trunk'] },
  { name: 'SPORTS PADDLE COVER', keywords: ['sports', 'paddle', 'cover'] },
  { name: 'STEERING WHEEL COVER', keywords: ['steering', 'wheel', 'cover'] },
  { name: 'BOOT MAT', keywords: ['boot', 'mat'] },
  { name: 'SEAT COVER', keywords: ['seat', 'cover'] },
  { name: 'DASH CAM', keywords: ['dash', 'cam'] },
  { name: 'BLAUPUNKT - KEY', keywords: ['blaupunkt', 'key'] },
...
protocol.get(url, (response) => {
  ...
  response.on('end', () => {
    ...
    if (images.length === 0) {
      ...
      const knownImages = [
        { url: allImages.find(img => img.includes('head') && img.includes('lamp')) || allImages[2], name: 'HEAD LAMP' },
        { url: allImages.find(img => img.includes('door') && (img.includes('side') || img.includes('moulding') || img.includes('molding'))) || allImages[3], name: 'DOOR SIDE MOULDING' },
        { url: allImages.find(img => img.includes('window') && img.includes('chrome')) || allImages[4], name: 'WINDOW LINE CHROME' },
        { url: allImages.find(img => img.includes('upper') && img.includes('trunk')) || allImages[5], name: 'UPPER TRUNK' },
        { url: allImages.find(img => img.includes('sports') || img.includes('paddle')) || allImages[6], name: 'SPORTS PADDLE COVER' },
        { url: allImages.find(img => img.includes('steering')) || allImages[7], name: 'STEERING WHEEL COVER' },
        { url: allImages.find(img => img.includes('boot') && img.includes('mat')) || allImages[8], name: 'BOOT MAT' },
        { url: allImages.find(img => img.includes('seat') && img.includes('cover')) || allImages[9], name: 'SEAT COVER' },
        { url: allImages.find(img => img.includes('dash') || img.includes('cam')) || allImages[10], name: 'DASH CAM' },
        { url: allImages.find(img => img.includes('blaupunkt') && img.includes('key')) || allImages[11], name: 'BLAUPUNKT - KEY' },
        { url: allImages.find(img => img.includes('blaupunkt') && !img.includes('key')) || allImages[12], name: 'BLAUPUNKT' },
        { url: allImages.find(img => img.includes('gt0609') || img.includes('gt0609c')) || allImages[13], name: 'GT0609C' },
