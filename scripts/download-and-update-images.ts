 ...
 */
function generateFilename(url: string, context: string): string {
  try {
 ...
 */
function getImageFolder(sourceFile: string): string {
 ...
 */
function extractImageUrls(filePath: string, content: string): UrlData[] {
  const urls: UrlData[] = [];
  const urlPatterns = [
 ...
 */
function scanFiles(): Map<string, FileData> {
  const imageMap = new Map<string, FileData>(); // filePath -> [{url, localPath, originalMatch}]
 ...
 */
function updateFileReferences(imageMap: Map<string, FileData>): number {
  let updatedFiles = 0;
