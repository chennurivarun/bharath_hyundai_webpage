...
const url = 'https://bharathyundai.com/tucson-accessories/';
const outputDir = path.join(__dirname, '..', 'public', 'images', 'accessories', 'tucson');
...
const accessories = [
  { name: 'HEAT CONTROL SUNROOF', keywords: ['heat', 'control', 'sunroof'] },
  { name: 'CUSHION', keywords: ['cushion'] },
  { name: 'HEAD REST CUSHION', keywords: ['head', 'rest', 'cushion'] },
...
protocol.get(url, (response) => {
  ...
  response.on('end', () => {
    ...
    if (images.length === 0) {
      ...
      const knownImages = [
        { url: allImages.find(img => img.includes('heat') && img.includes('sunroof')) || allImages[2], name: 'HEAT CONTROL SUNROOF' },
        { url: allImages.find(img => img.includes('cushion') && !img.includes('head')) || allImages[3], name: 'CUSHION' },
        { url: allImages.find(img => img.includes('head') && img.includes('cushion')) || allImages[4], name: 'HEAD REST CUSHION' },
        { url: allImages.find(img => img.includes('dash') || img.includes('cam')) || allImages[5], name: 'DASH CAM' },
