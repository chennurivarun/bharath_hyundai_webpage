...
const url = 'https://bharathyundai.com/aura-accessories/';
const outputDir = path.join(__dirname, '..', 'public', 'images', 'accessories', 'aura');
...
const accessories = [
  { name: 'DOOR HANDLE', keywords: ['door', 'handle'] },
  { name: 'WINDOW SIDE BEADING', keywords: ['window', 'beading'] },
  { name: 'WHEEL ARC', keywords: ['wheel', 'arc'] },
  { name: 'DOOR FINGER GUARD', keywords: ['door', 'finger', 'guard'] },
  ...
  { name: 'GLOVE BOX ORGANIZER', keywords: ['glove', 'box', 'organizer'] },
...
protocol.get(url, (response) => {
  ...
  response.on('end', () => {
    ...
    if (images.length === 0) {
      ...
      const knownImages = [
        { url: allImages.find(img => img.includes('door') && img.includes('handle')) || allImages[2], name: 'DOOR HANDLE' },
        { url: allImages.find(img => img.includes('window') || img.includes('beading')) || allImages[3], name: 'WINDOW SIDE BEADING' },
        { url: allImages.find(img => img.includes('wheel') && img.includes('arc')) || allImages[4], name: 'WHEEL ARC' },
        { url: allImages.find(img => img.includes('door') && img.includes('finger')) || allImages[5], name: 'DOOR FINGER GUARD' },
        { url: allImages.find(img => img.includes('steering')) || allImages[6], name: 'STEERING WHEEL COVER' },
        { url: allImages.find(img => img.includes('carpet') || img.includes('mat')) || allImages[7], name: 'CARPET MAT - PREMIUM' },
        { url: allImages.find(img => img.includes('seat') && img.includes('cover')) || allImages[8], name: 'SEAT COVER' },
        { url: allImages.find(img => img.includes('glove') || img.includes('organizer')) || allImages[9], name: 'GLOVE BOX ORGANIZER' },
        { url: allImages.find(img => img.includes('dash') || img.includes('cam')) || allImages[10], name: 'DASH CAM' },
        { url: allImages.find(img => img.includes('blaupunkt')) || allImages[11], name: 'BLAUPUNKT - KEY' },
        { url: allImages.find(img => img.includes('gt0609') || img.includes('gt0609c')) || allImages[12], name: 'GT0609C' },
