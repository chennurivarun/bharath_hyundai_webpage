...
interface Manifest {
  generatedAt: string;
  basePath: string;
  categories: Record<string, ManifestItem[]>;
}

const CATEGORY_HEADERS: CategoryHeader[] = [
...
function parseByCategory(raw: string): CategoryBuckets {
  const lines = raw.split(/\r?\n/);
  let current: keyof CategoryBuckets = 'highlights';
  const buckets: CategoryBuckets = { knight: [], highlights: [], exterior: [], interior: [], performance: [], safety: [], convenience: [] };
  for (const line of lines) {
    const cat = detectCategory(line);
    if (cat) { current = cat as keyof CategoryBuckets; continue; }
    const urls = splitConcatenatedUrls(line);
    for (const u of urls) buckets[current].push(u);
  }
  for (const key of Object.keys(buckets) as Array<keyof CategoryBuckets>) {
...
function toFileNameFromUrl(url: string, i: number = 0): string {
  const last = url.split('/').pop() || `image-${i}.jpg`;
  return sanitizeFileName(last);
}

function toCaptionFromUrl(url: string): string {
...
function absoluteRedirect(currentUrl: string, location: string): string {
  try {
...
async function main(): Promise<void> {
  ...
  const manifest: Manifest = { generatedAt: new Date().toISOString(), basePath: '/images/cars/i20/', categories: {} };
  for (const [key, list] of Object.entries(categories)) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        console.warn(`[skip] ${url} → ${fileName}: ${err.message}`);
      }
    }
