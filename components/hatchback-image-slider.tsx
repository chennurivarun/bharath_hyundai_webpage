...
const HATCHBACK_IMAGES = [
  {
    src: "/images/hatchback1.jpg",
    alt: "Hyundai Grand i10 NIOS - Red hatchback with sporty design",
    title: "Grand i10 NIOS",
    description: "Compact hatchback with modern features and premium styling"
  },
  {
    src: "/images/hatchback2.jpg", 
    alt: "Hyundai i20 - Red hatchback with dynamic styling",
    title: "i20",
    description: "Sporty hatchback with premium design and performance"
  }
]

export default function HatchbackImageSlider() {
  const [currentImage, setCurrentImage] = useState(0)
  const [isAutoPlaying, setIsAutoPlaying] = useState(true)
  const [imageErrors, setImageErrors] = useState<boolean[]>(new Array(HATCHBACK_IMAGES.length).fill(false))
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentImage((prev) => (prev + 1) % HATCHBACK_IMAGES.length)
    }, 3000) // Reduced to 3 seconds for faster sliding
  ...
  const prevImage = () => {
    setCurrentImage((prev) => (prev - 1 + HATCHBACK_IMAGES.length) % HATCHBACK_IMAGES.length)
    setIsAutoPlaying(false)
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full grayscale group-hover:grayscale-0 transition-all duration-500">
        {HATCHBACK_IMAGES.map((image, index) => (
          <div
            key={index}
