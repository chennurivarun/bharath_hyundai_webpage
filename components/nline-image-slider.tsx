...
const NLINE_IMAGES = [
  {
    src: "/images/i20-nline.avif",
    alt: "Hyundai i20 N Line - Sporty hatchback with performance styling",
    title: "i20 N Line",
    description: "Sporty hatchback with race-inspired design and enhanced performance"
  },
  {
    src: "/images/venue-nline.avif",
    alt: "Hyundai Venue N Line - Performance compact SUV",
    title: "Venue N Line",
    description: "Performance-tuned compact SUV with sporty styling"
  },
  {
    src: "/images/creta-nline.avif",
    alt: "Hyundai Creta N Line - Premium performance SUV",
    title: "Creta N Line",
    description: "Premium SUV with N Line performance enhancements"
  }
]

export default function NLineImageSlider() {
  const [currentImage, setCurrentImage] = useState(0)
  const [isAutoPlaying, setIsAutoPlaying] = useState(true)
  const [imageErrors, setImageErrors] = useState<boolean[]>(new Array(NLINE_IMAGES.length).fill(false))
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentImage((prev) => (prev + 1) % NLINE_IMAGES.length)
    }, 4000) // 4 seconds for N Line section
  ...
  const prevImage = () => {
    setCurrentImage((prev) => (prev - 1 + NLINE_IMAGES.length) % NLINE_IMAGES.length)
    setIsAutoPlaying(false)
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 left-2">
        <div className="bg-red-600/80 text-white text-xs px-2 py-1 rounded font-bold">
          N LINE
        </div>
      </div>
    </div>
